...
const sanitizeColorValue = (color: string) =>
  /^(#[0-9a-fA-F]{3,8}|rgba?\([^)]+\)|hsla?\([^)]+\)|var\(--[a-zA-Z0-9-]+\)|[a-zA-Z]+)$/.test(color) ? color : undefined;
...
${colorConfig
  .map(([key, itemConfig]) => {
    ...
    const color = rawColor ? sanitizeColorValue(rawColor) : undefined;
